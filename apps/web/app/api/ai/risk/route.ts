...
import { complianceRiskInputSchema } from "@sih/validation";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const body = complianceRiskInputSchema.parse(await req.json());
    assertMineAccess(ctx, body.mineId);

    const db = new SupabaseDb(getSupabaseServerClient());
    const ai = getAIService(db);
    const result = await ai.analyzeComplianceRisk(body);
